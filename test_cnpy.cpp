...
TEST_CASE("create_npy_header generates correct header for int type and "
  ...
  uint16_t dict_len =
      static_cast<uint16_t>(static_cast<unsigned char>(header[8]) |
...
TEST_CASE("parse_zip_footer correctly parses zip footer", "[cnpy]") {
  ...
  REQUIRE(file_size ==
          static_cast<long>(global_header_offset + global_header_size + 22));
...
// Additional unit tests for cnpy::npy_load with various data types
TEST_CASE("npy_load correctly loads a saved .npy file for unsigned short type",
          "[cnpy]") {
...
// Additional unit tests for cnpy::npy_load with various data types
TEST_CASE("npy_load correctly loads a saved .npy file for float type",
          "[cnpy]") {
...
TEST_CASE("npy_load correctly loads a saved .npy file for double type",
          "[cnpy]") {
...
TEST_CASE("npy_load correctly loads a saved .npy file for long double type",
          "[cnpy]") {
...
TEST_CASE("npy_load correctly loads a saved .npy file for complex<float> type",
          "[cnpy]") {
...
TEST_CASE("npy_load correctly loads a saved .npy file for unsigned char type",
          "[cnpy]") {
...
TEST_CASE("npy_load correctly loads a saved .npy file for long type",
          "[cnpy]") {
...
TEST_CASE("npz_load with two arrays loads correctly", "[cnpy]") {
  ...
  cnpy::npz_save<double>(filename, "double_arr", data_double.data(),
                         shape_double, "a");
  cnpy::npz_t arrays = cnpy::npz_load(filename);
...
TEST_CASE("npz_load with three arrays loads correctly", "[cnpy]") {
  ...
  cnpy::npz_save<float>(filename, "float_arr", data_float.data(), shape_float,
                        "a");
  cnpy::npz_t arrays = cnpy::npz_load(filename);
